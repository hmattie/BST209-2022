...
train_set <- reduced_data[train_index, ] %>% select(., age, bmi, gender, ethnicity, glucose_apache, icu_type, outcome_variable)
head(train_set)

test_set <- reduced_data[-train_index, ] %>% select(., age, bmi, gender, ethnicity, glucose_apache, icu_type, outcome_variable)
head(test_set)
```
...
forest <- randomForest(outcome_variable ~ age + bmi + gender + glucose_apache, 
              data = train_set,
              metric = "Accuracy",
              tuneGrid = tunegrid,
              trControl = control)
