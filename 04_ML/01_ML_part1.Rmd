---
title: 'Lecture: Machine Learning I'
output:
...
f_hat_251 <- predict(knn_fit_251, newdata = test_set)[,2]
tab <- table(pred=round(f_hat_251), truth=test_set$y)
confusionMatrix(tab)$overall["Accuracy"]
```
...
roc_knn_251 <- roc(test_set$y, f_hat_251)
plot(roc_knn_251)
```
...
```{r}
ggroc(list("kNN, k = 251" = roc_knn_251)) +
...
```{r}
fit <- knn3(y~., data = select(train_set, y, X_1, X_2), k = 5)
pred_knn_5 <- predict(fit, newdata = test_set)[,2]
roc_knn_5 <- roc(test_set$y, pred_knn_5)

ggroc(list("kNN, k = 5" = roc_knn_5, "kNN, k = 251" = roc_knn_251)) +
  theme(legend.title = element_blank()) +
  geom_segment(aes(x = 1, xend = 0, y = 0, yend = 1), color = "black", linetype = "dashed") +
  xlab("Specificity") +
  ylab("Sensitivity")
